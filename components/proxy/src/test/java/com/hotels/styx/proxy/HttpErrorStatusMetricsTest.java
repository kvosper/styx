...
//        assertThat(registry.get("backend.fault").counter(), is(notNullValue()));
//        assertThat(registry.get("backend.fault").counter().count(), is(0.0));
    }
    ...
    public void countsBackendFaults(Exception t, String metricTag) {
        ...
        if (counterWithMatchingTag("backend.fault", "faultType", metricTag).isEmpty()) {
            System.out.println(
                    registry.getMeters().stream()
                            .map(m -> m.getId() + " = " + m.measure()
                            )
                            .collect(joining("\n"))
            );
        }
        ...
        assertThat(
                counterWithMatchingTag("backend.fault", "faultType", metricTag)
                        .map(Counter::count),

                isValue(1.0));

//        // expects all tags to be specified. TODO do we want a different search?
//        assertThat(count("backend.fault", "faultType", metricTag), is(1));
    }

    private Optional<Counter> counterWithMatchingTag(String counterName, String tagKey, String tagValue) {
