...
fun countBackendFault(metrics: CentralisedMetrics, error: Throwable) : Boolean {
    metrics.run {
        when (error) {
            is NoAvailableHostsException ->
                countBackendFault(error.appId(), "noHostsLiveForApplication")
            is OriginUnreachableException ->
                countBackendFault(error.appId(), error.originId(), "cannotConnect")
            is BadHttpResponseException ->
                countBackendFault(error.appId(), error.originId(), "badHttpResponse")
            is MaxPendingConnectionTimeoutException ->
                countBackendFault(error.appId(), error.originId(), "connectionsHeldTooLong")
            is MaxPendingConnectionsExceededException ->
                countBackendFault(error.appId(), error.originId(), "tooManyConnections")
            is ResponseTimeoutException ->
                countBackendFault(error.appId(), error.originId(), "responseTooSlow")
            else -> return false
        }
    }
